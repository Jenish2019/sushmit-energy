import { connectDB, Project, BoardMember, ManagementMember, NewsArticle, Album, MediaResource, Report, Page, Contact, Setting } from './api';
...
export async function getProjects() {
  return withFallback(async () => {
    ...
    return docs.map((p) => ({ ...p, _id: String(p._id), capacity: formatCapacity(p.capacity) }));
  }, DEFAULTS.projects);
}
...
export async function getAllArticles() {
  ...
  return withFallback(async () => {
    const docs = await NewsArticle.find({ status: 'Published' }).sort({ createdAt: -1 }).lean();
    if (!docs.length) return fallback;
    return docs.map(mapArticle);
  }, fallback);
}
...
function mapAlbum(a) {
  ...
  const firstImg = images.length ? (typeof images[0] === 'string' ? images[0] : images[0]?.url || images[0]?.image || '') : '';
  return {
    name,
    cover: a.cover || firstImg,
...
export async function getPage(slug) {
  const fallbackMap = {
    ...
    projects: { title: 'Our Project', subtitle: 'Kunaban Khola Hydropower Project \u2014 20 MW in Myagdi District, Nepal' },
